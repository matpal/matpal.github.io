...
var Activity = App.Model({
                        ...
                        function(){
                            ...
                            if(servicesFound){
                                for (var i = 0; i < servicesFound.length; i++) {
                                    r = AndroidRect( 10,10,servicesFound[i],'service',AndroidComponents["service"].icon);
                            ...
                            if(intentsFound){
                                for (var i = 0; i < intentsFound.length; i++) {
                                    r = AndroidRect( 10,10,intentsFound[i].name,intentsFound[i].type,AndroidComponents[intentsFound[i].type].icon);
                                    curActivity.recsJava.add(r);
                            ...
                            if(receiversFound){
                                for (var i = 0; i < receiversFound.length; i++) {
                                    r = AndroidRect( 10,10,receiversFound[i],'broadcastReceiver',AndroidComponents['broadcastReceiver'].icon);
                                    curActivity.recsJava.add(r);
                            ...
                            if( providersFound ){
                                for (var i = 0; i < providersFound.length; i++) {
                                    r = AndroidRect( 10,10,providersFound[i],'contentProvider',AndroidComponents['contentProvider'].icon);
                                    curActivity.recsJava.add(r);
    ...
    parseLayoutTree: function(nodo, parent){
        ...
        var father = AndroidRect(
                    ...
                    rectType,
                    AndroidComponents[rectType].icon
                );
...
App.View('ExistingProject', {
    ...
    manifestDocumentTree: blocks.observable(),
    ...
    getComponent: function(component){
      if(!this.ManifestDocumentTree){
        ...
        console.log("parsing del manifest");
      }
        ...
        componentNameList.each(function(){
            ...
            if(intentsFound.length > 0){
                for(var j = 0; j< intentsFound.length; j++){
                    console.log("intanto " + $(intentsFound[j]).find('action').attr("android:name"));
                    temp.intents.push($(intentsFound[j]).find('action').attr("android:name"));
                }
                // temp.intents = $(this).find('intent-filter').find('action').attr("android:name").split('.').pop();
            }
    ...
    generateCode: function(){
        ...
        root.folder("java");
        //generate folder structure based on package name
